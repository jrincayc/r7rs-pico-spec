...
a memory location instead of a value.
...
variable binding construct is {\cf let}.
...
\schindex{number?}\schindex{procedure?}\index{empty list}
...
integer 28 also has representations ``{\tt +28}'', and the
list in the previous paragraph also has the representations ``{\tt( 08 13
)}'' (see section~\ref{listsection}).
